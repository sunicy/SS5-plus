...
UINT S5ReceiveConfig( struct _SS5ClientInfo *ci, struct _SS5Socks5Data *sd )
{
  ...
  if( STREQ(repMsg,"ROPAGATE_KEY\0",sizeof("ROPAGATE_KEY\0") - 1) ) {
    if( VERBOSE() )  {
  ...
  while( fscanf(S5ConfFile,"%255s",confString)!=EOF ) {
     ...
     */
      ...
      if( _tmp_NAclList < MAXACLLIST ) {
        ...
        if( inet_aton(pcf.srcAddr,&in) == 0 && inet_aton(pcf.dstAddr,&in) == 0 ) {
          ...
          S5StrHash(pcf.dstAddr),pcf.dstAddr,S5GetRange(pcf.dstPort),32-srcMask,32-dstMask,method,method2,&fa);
        }
        /*else if( pcf.dstAddr[0] >64 ) {*/
        else if(  inet_aton(pcf.dstAddr,&in) == 0) {
          SS5Modules.mod_authorization.AddAcl(OFFLINE,STREQ(confString,"permit",sizeof("permit") - 1)?PERMIT:DENY,inet_network(pcf.srcAddr),"-",
          S5GetRange(pcf.srcPort), S5StrHash(pcf.dstAddr),pcf.dstAddr,S5GetRange(pcf.dstPort),32-srcMask,32-dstMask,method,method2,&fa);
        }
        /*else if ( pcf.srcAddr[0] > 64 ) {*/
        else if ( inet_aton(pcf.srcAddr,&in) == 0) {
          ...
          inet_network(pcf.dstAddr),"-",S5GetRange(pcf.dstPort),32-srcMask,32-dstMask,method,method2,&fa);
        }
        else {
          SS5Modules.mod_authorization.AddAcl(OFFLINE,STREQ(confString,"permit",sizeof("permit") - 1)?PERMIT:DENY,inet_network(pcf.srcAddr),"-",
          S5GetRange(pcf.srcPort),inet_network(pcf.dstAddr),"-",S5GetRange(pcf.dstPort),32-srcMask,32-dstMask,method,method2,&fa);
        }
     ...
     */
      ...
      if( _tmp_NMethodList < MAXMETHODLIST ) {
        if( fscanf(S5ConfFile,"%20s %16s %3s\n",pcf.srcAddr,pcf.srcPort, pcf.socksMeth) < 3 ) {
          ERRNO(0)
          return ERR;
        }
        ...
        if( SS5Modules.mod_socks5.AddMethod(OFFLINE,inet_network(pcf.srcAddr),S5GetRange(pcf.srcPort),method,method2,32-srcMask) == ERR ) {
     ...
     */
      ...
      if( _tmp_NBandwidthList < MAXBANDLIST ) {
        if( fscanf(S5ConfFile,"%128s %6s %16s %16s\n",pcf.group,pcf.lCon,pcf.bandwidth,pcf.sessTimeout) < 4 ) {
          ERRNO(0)
          return ERR;
        }
        ...
        if( pcf.sessTimeout[0] == '-' )
          strncpy(pcf.sessTimeout,"0\0",2);
        ...
        if( pcf.group[0] != '-' ) {
          ...
          */
          ...
          while( fscanf(grpf,"%64s",pcf.user) != EOF ) {
            if( pcf.user[0] != '#' ) {
              if( _tmp_NBandwidthList < MAXBANDLIST ) {
                SS5Modules.mod_bandwidth.AddBandTable( OFFLINE, pcf.user, atoi(pcf.lCon), atol(pcf.bandwidth), atol(pcf.sessTimeout) );
                _tmp_NBandwidthList++;
              }
              else {
                if( m != PARSE_CONFIG )  {
     ...
     */
      ...
      if( SS5Modules.mod_dump_loaded ) {
        ...
        if( _tmp_NDumpList < MAXDUMPLIST ) {
          if( fscanf(S5ConfFile,"%64s %16s %1s %1s\n",pcf.dstAddr,pcf.dstPort,pcf.dumpDct,pcf.dumpDir) < 4 ) {
            ERRNO(0)
            return ERR;
          }
          ...
          //if( pcf.dstAddr[0] >64 ) {
          if( inet_aton(pcf.dstAddr,&in) == 0 ) {
            SS5Modules.mod_dump.AddDump(OFFLINE,S5StrHash(pcf.dstAddr),S5GetRange(pcf.dstPort),dct,sdr,32-dstMask);
          }
          else
            SS5Modules.mod_dump.AddDump(OFFLINE,inet_network(pcf.dstAddr),S5GetRange(pcf.dstPort),dct,sdr,32-dstMask);
